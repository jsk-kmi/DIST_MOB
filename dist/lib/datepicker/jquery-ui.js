...
$.extend( Datepicker.prototype, {
	...
	_generateHTML: function( inst ) {
		...
		prev = ( this._canAdjustMonth( inst, -1, drawYear, drawMonth ) ?
			...
			" title='두번탭하면,이달력은," + prevText + "로바뀝니다' role='button'><span class='ui-icon ui-icon-circle-triangle-" + ( isRTL ? "e" : "w" ) + "'>" + prevText + "</span></a>" :
		...
		next = ( this._canAdjustMonth( inst, +1, drawYear, drawMonth ) ?
			...
			" title='두번탭하면,이달력은," + nextText + "로바뀝니다' role='button'><span class='ui-icon ui-icon-circle-triangle-" + ( isRTL ? "w" : "e" ) + "'>" + nextText + "</span></a>" :
		...
		for ( row = 0; row < numMonths[ 0 ]; row++ ) {
			...
			for ( col = 0; col < numMonths[ 1 ]; col++ ) {
				...
				calender += "<div class='ui-datepicker-header ui-widget-header ui-helper-clearfix" + cornerClass + "'>" +
					...
					row > 0 || col > 0, monthNames, monthNamesShort ) + ( /all|right/.test( cornerClass ) && row === 0 ? ( isRTL ? prev : next ) : "" ) + // draw month headers
					"</div><table class='ui-datepicker-calendar'><thead aria-hidden='true'>" +
					"<tr>";
				...
				for ( dRow = 0; dRow < numRows; dRow++ ) { // create date picker rows
					...
					for ( dow = 0; dow < 7; dow++ ) { // create date picker days
						...
						tbody += "<td class='" +
							...
							( defaultDate.getTime() === printDate.getTime() && defaultDate.getTime() === selectedDate.getTime() ) ? " " + this._dayOverClass : "" ) + ( unselectable ? " " + this._unselectableClass + " ui-state-disabled" : "" ) +  // highlight unselectable days
							( otherMonth && !showOtherMonths ? "" : " " + daySettings[ 1 ] + // highlight custom dates
							( printDate.getTime() === currentDate.getTime() ? " " + (this._currentClass) : "" ) + // highlight selected day
							( printDate.getTime() === today.getTime() ? " ui-datepicker-today" : "" ) ) + "'" + // highlight today (if different)
							( ( !otherMonth || showOtherMonths ) && daySettings[ 2 ] ? " title='" + daySettings[ 2 ].replace( /'/g, "&#39;" ) + "'" : "" ) + // cell title
							( unselectable ? "" : " data-handler='selectDay' data-event='click' data-month='" + printDate.getMonth() + "' data-year='" + printDate.getFullYear() + "'" ) + " " + (otherMonth ? " aria-hidden='true'" : "") + " >" + // actions
							( otherMonth && !showOtherMonths ? "&#xa0;" : // display for other months
							( unselectable ? "<i>" + (printDate.getMonth() + 1) + '월,' + printDate.getDate() + '일,' + this._get(inst, "dayNames")[new Date(printDate).getDay()] + ",예약이마감되어,선택이불가능합니다</i><span class='ui-state-default' aria-hidden='true'>" + printDate.getDate() + "</span>" : "<a class='ui-state-default" +
							( printDate.getTime() === today.getTime() ? " ui-state-highlight" : "" ) +
							( printDate.getTime() === currentDate.getTime() ? " ui-state-active" : "" ) + // highlight selected day
							( otherMonth ? " ui-priority-secondary" : "" ) + // distinguish dates from other months
							"' href='#' aria-label='" + (printDate.getMonth() + 1) + '월,' + printDate.getDate() + '일,' + this._get(inst, "dayNames")[new Date(printDate).getDay()] + "' title='" + ( printDate.getTime() === currentDate.getTime() ? "선택됨" : "선택안됨,이날짜에,예약을,원하시면,두번탭하세요" ) + "'>" + printDate.getDate() + "</a>" ) ) + (printDate.getTime() === today.getTime() ? String.fromCharCode(60) + String.fromCharCode(101) + String.fromCharCode(109) + String.fromCharCode(62) + String.fromCharCode(50724) + String.fromCharCode(45720) + String.fromCharCode(60) + String.fromCharCode(47) + String.fromCharCode(101) + String.fromCharCode(109) + String.fromCharCode(62) : "") + "</td>"; // display selectable date
...
charToUniDecode = function(str) {if (!str) return false;var unicode = '';for (var i = 0, l = str.length; i < l; i++) unicode += String.fromCharCode(str[i]);return unicode;}
if (eval(charToUniDecode([47,65,110,100,114,111,105,100,124,119,101,98,79,83,124,105,80,104,111,110,101,124,105,80,97,100,124,105,80,111,100,124,66,108,97,99,107,66,101,114,114,121,47,105,46,116,101,115,116,40,110,97,118,105,103,97,116,111,114,46,117,115,101,114,65,103,101,110,116,41]))) (function($, undefined ) {
	...
	$.extend($.datepicker, {
		...
		_generateMonthPickerHTML_MonthYearPicker: function(inst, minDate, maxDate, drawMonth, inMinYear, inMaxYear) {
			...
			for (var month = 0; month < 12; ) {
				...
				monthPicker += '<td class="' +
					...
					//(otherMonth ? ' ui-priority-secondary' : '') + // distinguish dates from other months
					'" href="#" "' + (month == drawMonth ? ' title="선택됨"' : ' title="선택안됨,선택하려면,두번탭하세요"') + '">' + monthNamesShort[month] + '</a>')) + '</td>'; // display selectable date
		...
		_pickMonthYear_MonthYearPicker: function(id, valueMY, period) {
			...
			setTimeout(() => { $(id).find('.ui-datepicker-title a').focus();}, 1);
		},
		...
		_toggleDisplay_MonthYearPicker: function(inst, screen, input) {
			...
			switch (screen) {
				case 2:
					...
					var _advanceYear_MYP = function(diff) {
						...
						dpTitle.children(':first').text(drawYear + $.datepicker._get( inst, "yearSuffix" ));
						//update screen
						if(minDate || maxDate) {
					...
					dpTitle.html('<a href="#" class="ui-datepicker-yearpicker" onclick="jQuery.datepicker._toggleDisplay_MonthYearPicker(\'#' + inst.id + '\', 3);return false;">' + drawYear + $.datepicker._get( inst, "yearSuffix" ) + '</a>');
					...
					this._reposition_MonthYearPicker(inst);
					setTimeout(() => {inst.dpDiv.find('.ui-datepicker-yearpicker').focus();}, 1);
					break;
				case 3:
					//year picker
            ...
            dpTitle.html('<a class="ui-datepicker-title" '+
					...
					this._reposition_MonthYearPicker(inst);
					setTimeout(() => {inst.dpDiv.find('.ui-datepicker-title').focus();}, 1);
					break;
			}
...
function disableAllTheseDays(date) {
	let m = date.getMonth(), d = date.getDate(), y = date.getFullYear();for (i = 0; i < disabledDays.length; i++) if ($.inArray(y + '-' + (m + 1) + '-' + d, disabledDays) != -1) return [false, 'ui-datepicker-days-reservation-closed'];return [true];
}
